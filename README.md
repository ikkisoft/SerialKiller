...
When Java serialization is used to exchange information between a client and a server, attackers can try to replace the legitimate serialized stream with malicious data. Inspired by this [article](http://www.ibm.com/developerworks/library/se-lookahead/), SerialKillers inspects Java classes during naming resolution and allows a combination of blacklisting/whitelisting to secure your application.
...
1. Download the latest version of the [SerialKiller Jar](https://github.com/ikkisoft/SerialKiller/releases/)
...
In order to detect malicious payloads or allow your application's classes only, we need to use SerialKiller instead of the standard *java.io.ObjectInputStream*. This can be done with the following:
...
SerialKiller config supports the following settings:
 ...
 - **Refresh**: The refresh delay in milliseconds, used to *hot-reload* the configuration file. Good news for the devops! You don't need to restart your application if you change the config file
...
### Contributing
 ...
 - Submit a new RB, especially if you're aware of Java gadgets that can be abused by vulnerable applications. Providing a safe default configuration would be extremely useful to protect most of the applications out there
